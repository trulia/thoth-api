...
function dispatchServer(req,res){
...
if (information == 'integral'){
    ...
    var solrQueryInformation = createSolrRequest(req, filter);
    
    http.get(prepareHttpRequest(solrQueryInformation), function (resp) {
  ...
  //   solrQueryInformation.fl = thothFieldsMappings.count[attribute] +',tot-count_i,masterTime_dt' ;

  //   http.get(prepareHttpRequest(solrQueryInformation), function (resp) {
  //     prepareJsonResponse(res, resp, [thothFieldsMappings.count[attribute],'tot-count_i']);
  //   }).on("error", function(e){});   
...
function dispatch(req, res, entity){
  if (entity == 'server'){
    dispatchServer(req,res);
  }
}
